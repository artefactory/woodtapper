...
def _rules_filtering_stochastic(rules, probas, max_n_rules, n_features_in_, quantiles, random_state):
    """
    ...
    for ind_dim_abs in range(n_features_in_):
        ...
        if (list_categorical_indexes is not None) and (
            ...
            elem_low = (
                quantiles[:, ind_dim_continuous_array_quantile].min() - 1
            )
            elem_high = (
                quantiles[:, ind_dim_continuous_array_quantile].max() + 1
            )

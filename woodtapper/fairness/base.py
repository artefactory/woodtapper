...
def w2_fair_grad(y_preds, sensitive_attribute, n_steps_cdf=1024):
    ...
    if classes.size == 2:
        s_bin = np.where(sensitive_attribute == classes[0], 0, 1)
        return w2_fair_grad_binary(y_preds, s_bin, n_steps_cdf=n_steps_cdf)

    for group in classes:
        s_ovr = np.where(sensitive_attribute == group, 0, 1)
